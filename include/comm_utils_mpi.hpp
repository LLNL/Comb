...
inline void Recv_init(const void *buf, int count, MPI_Datatype mpi_type, int src, int tag, MPI_Comm comm, MPI_Request *request)
{
...
inline void Startall(int count, MPI_Request *array_of_requests)
{
  int ret = MPI_Startall(request);
  assert(ret == MPI_SUCCESS); 
}
