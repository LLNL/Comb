...
struct persistent_mpi_pol {
  ...
  static const char* get_name() { return "persistent_mpi"; }
...
template < >
struct CommContext<persistent_mpi_pol> : MPIContext
{
  using base = MPIContext;

  using pol = persistent_mpi_pol;
...
template < >
struct Message<MessageBase::Kind::send, persistent_mpi_pol>
  : MessageInterface<MessageBase::Kind::send, persistent_mpi_pol>
{
  using base = MessageInterface<MessageBase::Kind::send, persistent_mpi_pol>;
...
template < >
struct Message<MessageBase::Kind::recv, persistent_mpi_pol>
  : MessageInterface<MessageBase::Kind::recv, persistent_mpi_pol>
{
  using base = MessageInterface<MessageBase::Kind::recv, persistent_mpi_pol>;
...
struct MessageGroup<MessageBase::Kind::send, persistent_mpi_pol, exec_policy>
  : detail::MessageGroupInterface<MessageBase::Kind::send, persistent_mpi_pol, exec_policy>
{
  using base = detail::MessageGroupInterface<MessageBase::Kind::send, persistent_mpi_pol, exec_policy>;
  ...
  void setup(context_type& con, communicator_type& con_comm, message_type** msgs, IdxT len)
  {
    COMB::ignore_unused(con, con_comm);
    LOGPRINTF("%p initialize persistent communication\n", this);
    for (IdxT i = 0; i < len; i++) {// should it be ++i?
      ...
      detail::MPI::Send_init(buf, msg_nbytes, MPI_BYTE, partner_rank, tag, con_comm.comm, &(msg->request));
    }
  }

  void cleanup(context_type& con, communicator_type& con_comm, request_type *requests, IdxT len)
  {
  ...
  void Isend(context_type& con, communicator_type& con_comm, message_type** msgs, IdxT len, detail::Async /*async*/, request_type* requests)
  {
    ...
    for (IdxT i = 0; i < len; ++i) {
      ...
      request_type *request = msg->request;

      detail::MPI::Start(request);
    }
  ...
  : detail::MessageGroupInterface<MessageBase::Kind::recv, mpi_pol, exec_policy>
{
  ...
  void setup(context_type& con, communicator_type& con_comm, message_type** msgs, IdxT len)
  {
    ...
    for (IdxT i = 0; i < len; i++) {// should it be ++i?
      ...
      const int tag = msg->msg_tag;
      const IdxT msg_nbytes = msg->nbytes() * this->m_variables.size();
      detail::MPI::Recv_init(buf, msg_nbytes, MPI_BYTE, partner_rank, tag, con_comm.comm, &(msg->request));
    }
  }
  ...
  {
    COMB::ignore_unused(con, con_comm, requests);
    ...
    for (IdxT i = 0; i < len; ++i) {
      ...
      LOGPRINTF("%p recv Irecv msg %p buf %p nbytes %d to %d tag %d\n",
                                this, msg, msg->buf, msg->nbytes() * this->m_variables.size(), msg->partner_rank, msg->msg_tag);
      const request_type *request = msg->request; 
      detail::MPI::Start(request);
    }
  }
