...
#ifndef _COMM_POL_GDSYNC_HPP
#define _COMM_POL_GDSYNC_HPP
...
struct mempool
{
  ...
  ibv_ptr allocate(struct gdsync* g, COMB::Allocator& aloc_in, size_t size)
  {
    assert(g == this->g);
    ibv_ptr ptr{};
    if (size > 0) {
      ...
      if (iter != m_allocators.end()) {
        ...
        if (unused_iter != unused_ptrs.end()) {
          ...
          info.ptr.mr = detail::gdsync::register_region(g, info.ptr.ptr, info.size);
          info.ptr.offset = 0;
          used_ptrs.emplace(info.ptr.ptr, info);
        }
        ...
        throw std::invalid_argument("unknown allocator passed to detail::gdsync::mempool::allocate");
      }
    }
    return ptr;
  }

  void deallocate(struct gdsync* g, COMB::Allocator& aloc_in, ibv_ptr ptr)
  {
    assert(g == this->g);
    if (ptr.ptr != nullptr) {
      ...
      if (iter != m_allocators.end()) {
        ...
        if (used_iter != used_ptrs.end()) {
          ...
          throw std::invalid_argument("unknown or unused pointer passed to detail::gdsync::mempool::deallocate");
        }
      } else {
        throw std::invalid_argument("unknown allocator passed to detail::gdsync::mempool::deallocate");
      }
    }
  }

  void add_allocator(struct gdsync* g, COMB::Allocator& aloc)
  {
    if (this->g == nullptr) {
      this->g = g;
    }
  ...
  void remove_allocators(struct gdsync* g)
  {
    assert(g == this->g);
    bool error = false;
    ...
    while (iter != m_allocators.end()) {
      ...
      while (inner_iter != unused_ptrs.end()) {
        ...
        detail::gdsync::deregister_region(this->g, info.ptr.mr);
...
template < >
struct CommContext<gdsync_pol> : CudaContext
{
  using base = CudaContext;

  using pol = gdsync_pol;
  ...
  CommContext(CommContext const& a_, MPI_Comm comm_)
    : base(a_)
    , g(detail::gdsync::init(comm_))
  { }

  ~CommContext()
  {
    if (g != nullptr) {
      detail::gdsync::term(g); g = nullptr;
    }
  }
                     ...
                     std::vector<int> const& recv_ranks)
  {
    ...
    for (int rank : ranks) {
      detail::gdsync::connect_propose(g, rank);
    }
    for (int rank : ranks) {
      detail::gdsync::connect_accept(g, rank);
    }
  }
                        ...
                        std::vector<int> const& recv_ranks)
  {
    ...
    for (int rank : ranks) {
      detail::gdsync::disconnect(g, rank);
    }
  }


  inline detail::gdsync::mempool& get_mempool()
  {
    static detail::gdsync::mempool mempool;
    return mempool;
  }
  ...
  {
    using region_type = detail::gdsync::mempool::ibv_ptr;

    detail::MessageBase::Kind kind;
    region_type region;
    detail::gdsync::Request request;
  ...
  void wait_request(message_request_type* msg_request)
  {
    ...
    while (1) {
      for (message_request_type* other_msg_request : get_message_request_map()) {
        ...
        detail::gdsync::Request& other_request = other_msg_request->request;

        if (!other_request.completed && other_kind == msg_request->kind) {
          if (other_kind == detail::MessageBase::Kind::send) {
            other_request.completed = detail::gdsync::is_send_complete(other_request.g, other_request.partner_rank);
          } else if (other_kind == detail::MessageBase::Kind::recv) {
            other_request.completed = detail::gdsync::is_receive_complete(other_request.g, other_request.partner_rank);
          } else {
...
template < >
struct Message<MessageBase::Kind::send, gdsync_pol>
  : MessageInterface<MessageBase::Kind::send, gdsync_pol>
{
  using base = MessageInterface<MessageBase::Kind::send, gdsync_pol>;
                            ...
                            int* indices, status_type* statuses)
  {
    ...
    if (count > 0) {
      ...
      if (new_requests) {
        // detail::gdsync::uncork(con_comm.g, con_comm.stream_launch());
      }
    }
    return done;
  }
...
private:
                              ...
                              request_type& request)
  {
    ...
    if (request->context_type == ContextEnum::cuda) {
      detail::gdsync::stream_wait_send_complete(request->g, request->partner_rank, request->context.cuda.stream_launch());
      done = true;
    } else if (request->context_type == ContextEnum::cpu) {
      detail::gdsync::cpu_ack_isend(request->g, request->partner_rank);
    } else {
                                ...
                                request_type& request)
  {
    ...
    bool done = false;
    if (request->context_type == ContextEnum::cuda) {
      done = detail::gdsync::is_send_complete(request->g, request->partner_rank);
...
template < >
struct Message<MessageBase::Kind::recv, gdsync_pol>
  : MessageInterface<MessageBase::Kind::recv, gdsync_pol>
{
  using base = MessageInterface<MessageBase::Kind::recv, gdsync_pol>;
...
private:
                              ...
                              request_type& request)
  {
    ...
    bool done = false;
    if (request->context_type == ContextEnum::cuda) {
      detail::gdsync::stream_wait_recv_complete(request->g, request->partner_rank, request->context.cuda.stream_launch());
      done = true;
    } else if (request->context_type == ContextEnum::cpu) {
      detail::gdsync::cpu_ack_recv(request->g, request->partner_rank);
    } else {
      assert(0 && (request->context_type == ContextEnum::cuda || request->context_type == ContextEnum::cpu));
    }
    return done;
                                ...
                                request_type& request)
  {
    ...
    bool done = false;
    if (request->context_type == ContextEnum::cuda) {
      done = detail::gdsync::is_receive_complete(request->g, request->partner_rank);
      request->completed = done;
...
struct MessageGroup<MessageBase::Kind::send, gdsync_pol, exec_policy>
  : detail::MessageGroupInterface<MessageBase::Kind::send, gdsync_pol, exec_policy>
{
  using base = detail::MessageGroupInterface<MessageBase::Kind::send, gdsync_pol, exec_policy>;
      ...
      for (IdxT i = 0; i < len; ++i) {
        ...
        for (const MessageItemBase* msg_item : msg->message_items) {
          ...
          for (DataT const* src : this->m_variables) {
            ...
            this->m_contexts[msg_idx].for_all(0, nitems, make_copy_idxr_idxr(src, detail::indexer_list_idx{indices},
...
private:
  ...
  {
    detail::gdsync::isend(con_comm.g, partner_rank, msg_request.region.mr, msg_request.region.offset, nbytes);
  }
  ...
  {
    detail::gdsync::stream_send(con_comm.g, partner_rank, con.stream_launch(), msg_request.region.mr, msg_request.region.offset, nbytes);
  }
  ...
  void finish_send(communicator_type& con_comm, message_request_type& msg_request)
  {
    if (!msg_request.request.completed) {
      msg_request.request.completed = detail::gdsync::is_send_complete(msg_request.request.g, msg_request.request.partner_rank);
...
template < typename exec_policy >
struct MessageGroup<MessageBase::Kind::recv, gdsync_pol, exec_policy>
  : detail::MessageGroupInterface<MessageBase::Kind::recv, gdsync_pol, exec_policy>
{
  using base = detail::MessageGroupInterface<MessageBase::Kind::recv, gdsync_pol, exec_policy>;
      ...
      for (IdxT i = 0; i < len; ++i) {
        ...
        for (const MessageItemBase* msg_item : msg->message_items) {
          ...
          for (DataT* dst : this->m_variables) {
            ...
            this->m_contexts[msg->idx].for_all(0, nitems, make_copy_idxr_idxr(static_cast<DataT const*>(static_cast<void const*>(buf)), detail::indexer_idx{},
...
private:
  ...
  {
    if (!msg_request.request.completed) {
      msg_request.request.completed = detail::gdsync::is_receive_complete(msg_request.request.g, msg_request.request.partner_rank);
...
template < >
struct MessageGroup<MessageBase::Kind::send, gdsync_pol, mpi_type_pol>
{
  // unimplemented
};

template < >
struct MessageGroup<MessageBase::Kind::recv, gdsync_pol, mpi_type_pol>
{
  // unimplemented
};
