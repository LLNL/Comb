...
{
  // FGPRINTF(FileGroup::proc, "gpump_init rank(w%i)\n", MPI::Comm_rank(MPI_COMM_WORLD));
  struct ::gpump* g = gpump_init(mpi_comm);
  // FGPRINTF(FileGroup::proc, "gpump_init rank(w%i) done -> %p\n", MPI::Comm_rank(MPI_COMM_WORLD), g);
  assert(g != nullptr);
  return g;
}
...
{
  // FGPRINTF(FileGroup::proc, "gpump_stream_send(%p) rank(w%i) %p+%zu[%zu] dst(%i) stream(%p)\n", g, MPI::Comm_rank(MPI_COMM_WORLD), buf_mr, offset, size, dest, (void*)stream);
